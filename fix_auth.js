...
fs.writeFileSync('src/app/auth/signup/page.tsx', `'use client'
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0a0a0a',display:'flex',alignItems:'center',justifyContent:'center',padding:'32px'}}>
      <div style={{width:'100%',maxWidth:'420px'}}>
        ...
        <h1 style={{fontFamily:'Georgia,serif',fontSize:'36px',fontWeight:'700',color:'#f5f0e8',marginBottom:'8px',letterSpacing:'-1px'}}>Create account</h1>
        <p style={{color:'#555',fontSize:'15px',marginBottom:'32px'}}>Join and start sharing your ideas</p>
        ...
        <form onSubmit={handleSignup} style={{display:'flex',flexDirection:'column',gap:'16px'}}>
          <div>
            <label style={{display:'block',color:'#555',fontSize:'12px',letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:'8px'}}>Full Name</label>
          ...
          <div>
            <label style={{display:'block',color:'#555',fontSize:'12px',letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:'8px'}}>I want to</label>
            <select value={role} onChange={e => setRole(e.target.value)}
              style={{width:'100%',background:'#111',border:'1px solid #222',borderRadius:'6px',padding:'14px 16px',fontSize:'15px',color:'#f5f0e8',outline:'none',boxSizing:'border-box'}}>
              <option value='viewer'>Read and comment</option>
        ...
        <p style={{textAlign:'center',marginTop:'24px',color:'#444',fontSize:'14px'}}>
          Already have an account? <Link href='/auth/login' style={{color:'#e8c547',textDecoration:'none',fontWeight:'600'}}>Sign in</Link>
        </p>
      </div>
    </div>
  )
